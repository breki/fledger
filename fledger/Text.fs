...
/// <see cref="System.Text.StringBuilder" /> instance.
/// </summary>
let appendChar (chr: char) (sb: StringBuilder) = sb.Append(chr)

let appendLine (text: string) (sb: StringBuilder) = sb.AppendLine(text)

let appendLines lines (sb: StringBuilder) =
    lines |> Seq.fold (fun builder line -> builder |> appendLine line) sb

let appendFormat format ([<ParamArray>] args: obj[]) (sb: StringBuilder) =
    sb.AppendFormat(format, args)

let newLine (sb: StringBuilder) = sb.AppendLine()

let ifDo condition work sb = if condition then sb |> work else sb

let ifDont condition work sb =
    if not condition then sb |> work else sb
